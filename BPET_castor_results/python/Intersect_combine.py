...
# Option to save files - save_fracs
###
...
# Variables to store the points of intersection
pts_intersect_all = []
times_intersect_all = []
...
for i in range(nvalue):
    ...
    # Append each fraction to the same variable
    pts_intersect_all.append(pts_intersect_small)
    times_intersect_all.append(times_intersect_small)

##############################################
# Combine the points of intersection
##############################################

# Combine the points of intersection
pts_intersect_all = np.concatenate(pts_intersect_all)
times_intersect_all = np.concatenate(times_intersect_all)
Console().print('Number of points after combining all fractions:', pts_intersect_all.shape)

##############################################
# Save the points of intersection
##############################################

Console().print('Saving all points of intersection...')
data_to_save = np.column_stack((times_intersect_all, pts_intersect_all[:, 0, 0], pts_intersect_all[:, 0, 1], pts_intersect_all[:, 0, 2], pts_intersect_all[:, 1, 0], pts_intersect_all[:, 1, 1], pts_intersect_all[:, 1, 2]))
# Save the points of intersection along with time, x, y, and z coordinates to a text file
np.savetxt('../Files/LOR_all_points_time_small.txt', data_to_save, header='time (msec), x1, y1, z1, x2, y2, z2', delimiter=',')
Console(style='bold green').print('File saved: LOR_all_points_time_small.txt')
